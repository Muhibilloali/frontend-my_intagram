...
import { getUserPhotosByUId, deletePhoto } from "../../services/services";
...
const UserPhotos = ({ user, setTotalPost }) => {
    ...
    const handleDeletePhoto = async (photoId) => {
        try {
            ...
            setUserPhotos(userPhotos.filter(photo => photo.photoDocId !== photoId));
            
            setTotalPost(userPhotos.length - 1);
        } catch (error) {
                    ...
                    {userPhotos.map((item) => (
                        <div className="relative group" key={item.photoDocId}>
                            ...
                            <div className="absolute bottom-0 left-0 bg-gray-200 z-10 w-full justify-evenly items-center h-full bg-black-faded group-hover:flex hidden">
                                ...
                                <button
                                        onClick={() => handleDeletePhoto(item.photoDocId)}
